...
const getVehicleWithPrice = async (vehicleId) => {
    const result = await db_1.pool.query(`SELECT id, vehicle_name, daily_rent_price, availability_status
     FROM vehicles
...
const getBookingById = async (bookingId) => {
    const result = await db_1.pool.query(`SELECT * FROM bookings WHERE id = $1`, [
        bookingId,
    ]);
    return result.rows[0];
};
